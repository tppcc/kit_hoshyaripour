...
! D2A(1) = d^2 A(2) / dV(1)dV(3)
  D2A(1) = 1.8e-12
! D2A(2) = d^2 A(3)/{dV(2)dV(2)}
  D2A(2) = 1.5e-12*2
! D2A(3) = d^2 A(4) / dV(1)dV(2)
  D2A(3) = 1.1e-10

! Computation of the Jacobian derivative
! HESS(1) = d^2 Vdot(1)/{dV(1)dV(2)} = d^2 Vdot(1)/{dV(2)dV(1)}
  HESS(1) = -D2A(3)
! HESS(2) = d^2 Vdot(1)/{dV(1)dV(3)} = d^2 Vdot(1)/{dV(3)dV(1)}
  HESS(2) = -D2A(1)
! HESS(3) = d^2 Vdot(2)/{dV(1)dV(2)} = d^2 Vdot(2)/{dV(2)dV(1)}
  HESS(3) = -D2A(3)
! HESS(4) = d^2 Vdot(2)/{dV(1)dV(3)} = d^2 Vdot(2)/{dV(3)dV(1)}
  HESS(4) = D2A(1)
! HESS(5) = d^2 Vdot(2)/{dV(2)dV(2)} = d^2 Vdot(2)/{dV(2)dV(2)}
  HESS(5) = -2*D2A(2)
! HESS(6) = d^2 Vdot(3)/{dV(1)dV(3)} = d^2 Vdot(3)/{dV(3)dV(1)}
  HESS(6) = -D2A(1)
! HESS(7) = d^2 Vdot(3)/{dV(2)dV(2)} = d^2 Vdot(3)/{dV(2)dV(2)}
  HESS(7) = D2A(2)
...
! Compute the vector HTU =(Hess x U2)^T * U1 = d (Jac^T*U1)/d Var * U2
  HTU(1) = HESS(1)*(U1(1)*U2(2))+HESS(2)*(U1(1)*U2(3))+HESS(3)*(U1(2)*U2(2))+HESS(4)*(U1(2)*U2(3))+HESS(6)*(U1(3)*U2(3))
  HTU(2) = HESS(1)*(U1(1)*U2(1))+HESS(3)*(U1(2)*U2(1))+HESS(5)*(U1(2)*U2(2))+HESS(7)*(U1(3)*U2(2))
  HTU(3) = HESS(2)*(U1(1)*U2(1))+HESS(4)*(U1(2)*U2(1))+HESS(6)*(U1(3)*U2(1))
...
! Compute the vector HU =(Hess x U2) * U1 = d (Jac*U1)/d Var * U2
  HU(1) = HESS(1)*(U1(1)*U2(2))+HESS(1)*(U1(2)*U2(1))+HESS(2)*(U1(1)*U2(3))+HESS(2)*(U1(3)*U2(1))
  HU(2) = HESS(3)*(U1(1)*U2(2))+HESS(3)*(U1(2)*U2(1))+HESS(4)*(U1(1)*U2(3))+HESS(4)*(U1(3)*U2(1))+HESS(5)*(U1(2)*U2(2))
  HU(3) = HESS(6)*(U1(1)*U2(3))+HESS(6)*(U1(3)*U2(1))+HESS(7)*(U1(2)*U2(2))
