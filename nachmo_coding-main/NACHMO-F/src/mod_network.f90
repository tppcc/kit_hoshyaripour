module mod_network
  ...
  contains
    ...
    procedure, public, pass(self) :: prnt
...
contains
  ...
  pure real(rk) function accuracy(self, x, y)
    ...
    L2 = 0
    do i=1, size(x, dim = 2)
    yt = self % output(x(:,i)) !size(self % output(x(:,i)))
    
    
    
    L2 = L2+ (yt(1) - y(1,i))*(yt(1) - y(1,i))
...
!  end function output_single
  ...
  pure function output_single(self, x) result(a)
    ...
    integer(ik) :: n
    associate(layers => self % layers)
      a = self % layers(2) % activation(matmul(transpose(layers(2) % w), x) + layers(2) % b)
      do n = 3, size(layers)
        a = self % layers(n) % activation(matmul(transpose(layers(n) % w), a) + layers(n) % b)
      end do
    end associate
!    call prnt(x)
  end function output_single
  ...
  subroutine prnt(self, x )
  class(network_type), intent(in out) :: self
    real, intent(in) :: x(:)  
    print *, x
    print *, '==================================='
!    open( unit=20, file="../data/Pangea/tmp.txt")
!    write(unit=20,fmt=*) x
!    close(20)
  end subroutine prnt
